...
class ExprNeg(unittest.TestCase):
    def test_run_addition(self):
        ...
        for exprCode in ["1--", "+1+", "2-4/5%", "5%5%"]:
            TestEzhilException.create_and_test(exprCode, ParseException,
                                               "Expected Number")
...
முடி""" #error message is bad for nested function case. but alteast it errors out.
        TestEzhilException.create_and_test(
            exprCode, ParseException, "Parse error: cannot find token END")
...
நிரல்பாகம்  fact ( n )
  @( n == 0 )  இல்லை
    ...
    முடி""" #missing end function statement
        TestEzhilException.create_and_test(
            exprCode, ParseException, "parsing Statement, unknown operators")
...
"""
        ...
        expected = "Function '%s' expects %d arguments but received only %d" % (
            "function", 1, 0)
