...
class TestEzhil:
    ...
    def __str__(self):
        if (self._tested):
            return "******* test completed with status " + str(
                self.success) + " *********"
    ...
    def run(self):
        ...
        self._tested = True
        try:
            ezhil.EzhilFileExecuter(file_input=self.filename,
                                    debug=False,
                                    redirectop=False)
            self.success = True
    ...
    def run(self):
        ...
        try:
            ...
            if not self.success:
                traceback.print_stack()
                raise Exception(
                    "Expected exception class %s was not found; instead %s was received."
                    % (self.exception, ex))
            ...
            if not self.success and not self.partial:
                ...
                raise Exception(
                    "Expected message %s was not found. We found message %s" %
                    (self.message, str(ex)))
            return self.success
        raise Exception("Expected message %s was not found.",
                        str(self.exception))

    @staticmethod
    def create_and_test_spl_safe_mode(code, exception, msg):
        with TestEzhilException(code,
                                exception,
                                msg,
                                debug=False,
                                safe_mode=True) as tst:
            flag = tst.run()
            print(tst)
        return flag
...
class TestTimeoutEzhil(TestEzhil):
    ...
    def run(self):
        """ run the interpreter """
        print((
            "\n******* beginning to run Ezhil test with timeout = %d(s)*******"
            % (self.timeout)))
        self._tested = True
        self.success = False
        try:
            #redirect output = True, when you need a TIMEOUT
            ezhil.EzhilFileExecuter(self.filename,
                                    debug=False,
                                    redirectop=True,
