...
class BadLexingTests(QuietTestCase):
        ...
        for idx in range(limit):
            ...
            for idy in range(idlen):
                if choice([2, 3]) % 2 == 0:
                    ...
                    newID.append(
                        choice(BadLexingTests.FORBIDDEN_FOR_IDENTIFIERS))
            yield "".join(newID)
        raise StopIteration
    ...
    def isNotForbidden(wrongID):
        return all(
            [x for x in wrongID if not x in BadLexingTests.FORBIDDEN_FOR_IDENTIFIERS])

    def test_id(self):
        for idx, wrongID in enumerate(
                BadLexingTests.pattgen(2 * choice([100, 250, 350]))):
            flag = False
            try:
