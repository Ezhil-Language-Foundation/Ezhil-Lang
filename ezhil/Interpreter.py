...
def ezhil_keywords():
    keywords = {
        ...
        u"இல்லை": "ELSE",
        u"ஆக": "FOR",
        u"ஒவ்வொன்றாக": "FOREACH",
        u"இல்": "COMMA",
        u"வரை": "WHILE",
        u"செய்": "DO",
        u"முடியேனில்": "DOWHILE",
        u"பின்கொடு": "RETURN",
        u"முடி": "END",
        u"நிரல்பாகம்": "DEF",
        u"தொடர்": "CONTINUE",
...
def ezhil_getitem(*args):
    if len(args) < 2:
        raise Exception(
            "getitem - too-few arguments, 2 or 3 arguments expected")
...
class NoClobberDict(dict):
    ...
    def __setitem__(self, key, val):
        if (self.get(key, None)):
            ...
            raise KeyError(u"Dictionary is getting clobbered; key '" + key +
                           "' already present '%s'" % unicode(self.get(key)))
...
def ezhil_eval(*args):
    ...
    parse_eval = Interpreter(lexer,
                             debug,
                             global_interpreter.SAFE_MODE,
                             False,
                             env=global_interpreter.env)
    try:
...
def ezhil_profile(*args):
    ...
    if not env.profiler:
        raise RuntimeException(
            u"profile begin should be called before other profile commands like end, results etc"
        )
    ...
    raise RuntimeException(u"profile command used with unknown argument %s" %
                           args[0])
...
class Interpreter(DebugUtils):
    ...
    def file_open(*args):
        try:
            ...
            print(u"WARNING : cannot open file %s with exception %s", args[0],
                  str(e))
        return -1
    ...
    def add_builtin(self, call_name, call_handle, nargin=1, ta_alias=None):
        ...
        if (nargin == -1):
            self.builtin_map[call_name] = BlindBuiltins(
                call_handle, call_name, self.debug)
        else:
            self.builtin_map[call_name] = BuiltinFunction(
                call_handle, call_name, nargin, self.debug)
    ...
    def install_builtins(self):
        ...
        self.builtin_map[u'printf'] = BlindBuiltins(Interpreter.PRINTF,
                                                    u'printf', self.debug)
        self.builtin_map[u'sprintf'] = BlindBuiltins(Interpreter.SPRINTF,
                                                     u'sprintf', self.debug)
        ...
        if not PYTHON3:
            self.builtin_map['apply'] = BlindBuiltins(apply, 'apply',
                                                      self.debug)
        ...
        self.builtin_map['eval'] = BlindBuiltins(ezhil_eval, 'eval',
                                                 self.debug)
        self.builtin_map[u'மதிப்பீடு'] = self.builtin_map['eval']
        self.builtin_map['execute'] = BlindBuiltins(ezhil_execute, 'execute',
                                                    self.debug)
        self.builtin_map[u'இயக்கு'] = self.builtin_map['execute']

        self.builtin_map['profile'] = BlindBuiltins(ezhil_profile, 'profile',
                                                    self.debug)
        ...
        try:
            self.builtin_map['filter'] = BlindBuiltins(filter, 'filter',
                                                       self.debug)
            self.builtin_map['float'] = BlindBuiltins(float, 'float',
                                                      self.debug)
            self.builtin_map['format'] = BlindBuiltins(format, 'format',
                                                       self.debug)
            self.builtin_map['frozenset'] = BlindBuiltins(
                frozenset, 'frozenset', self.debug)
            self.builtin_map['getattr'] = BlindBuiltins(
                getattr, 'getattr', self.debug)
            #self.builtin_map['globals']=BlindBuiltins(globals,'globals',self.debug)
            self.builtin_map['hasattr'] = BlindBuiltins(
                hasattr, 'hasattr', self.debug)
            ...
            self.builtin_map['isinstance'] = BlindBuiltins(
                isinstance, 'isinstance', self.debug)
            self.builtin_map['issubclass'] = BlindBuiltins(
                issubclass, 'issubclass', self.debug)
            self.builtin_map['iter'] = BlindBuiltins(iter, 'iter', self.debug)
            self.builtin_map['len'] = BlindBuiltins(len, 'len', self.debug)
            self.builtin_map['license'] = BlindBuiltins(
                ezhil_license, 'license', self.debug)
        ...
        if not PYTHON3:
            ...
            self.builtin_map['reload'] = BlindBuiltins(reload, 'reload',
                                                       self.debug)
            self.builtin_map['unicode'] = BlindBuiltins(
                str, 'unicode', self.debug)
        ...
        self.builtin_map['__getitem__'] = BlindBuiltins(
            ezhil_getitem, "__getitem__")
        ...
        # keywords
        self.add_builtin("keywords",
                         lambda: ezhil_keywords()[0],
                         nargin=0,
                         ta_alias=u"குறிசொற்கள்")
        ...
        self.builtin_map["sys_platform"] = BuiltinFunction(
            lambda: sys.platform, "sys_platform", padic=0)
        ...
        if not PYTHON3:
            ...
            self.builtin_map["capwords"] = BuiltinFunction(
                string.capwords, "capwords", 1)
            self.builtin_map["center"] = BuiltinFunction(
        ...
        self.builtin_map["swapcase"] = BuiltinFunction(str.swapcase,
                                                       "swapcase", 1)
        self.builtin_map["translate"] = BuiltinFunction(
            str.translate, "translate", 1)
        ...
        if not PYTHON3:
            self.builtin_map["has_key"] = BuiltinFunction(
                dict.has_key, "has_key", 1)
            self.builtin_map["iteritems"] = BuiltinFunction(
                dict.iteritems, "iteritems", 1)
            self.builtin_map["iterkeys"] = BuiltinFunction(
                dict.iterkeys, "iterkeys", 1)
            ...
            self.builtin_map["popitem"] = BuiltinFunction(
                dict.popitem, "popitem", 1)
            self.builtin_map["setdefault"] = BuiltinFunction(
                dict.setdefault, "setdefault", 1)

        if PYTHON3:
            self.builtin_map["keys"] = BuiltinFunction(
                lambda x: list(dict.keys(x)), "keys", 1)
            self.builtin_map["values"] = BuiltinFunction(
                lambda x: list(dict.values(x)), "values", 1)
        else:
            self.builtin_map["keys"] = BuiltinFunction(dict.keys, "keys", 1)
            self.builtin_map["values"] = BuiltinFunction(
                dict.values, "values", 1)
        ...
        self.add_builtin("get_tamil_letters",
                         ...
                         nargin=1,
                         ta_alias=u"தமிழ்_எழுத்துக்கள்")
        self.add_builtin(u"த", tamil.utf8.get_letters, nargin=1)
        self.add_builtin("tamil_length",
                         ezhil_tamil_length,
                         nargin=1,
                         ta_alias=u"தநீளம்")

        # functions returning constant list of Tamil strings
        self.add_builtin("tamil_letters",
                         lambda: tamil.utf8.tamil_letters,
                         nargin=0,
                         ta_alias=u"தமிழ்எழுத்து")
        self.add_builtin("tamil_uyir",
                         lambda: tamil.utf8.uyir_letters,
                         nargin=0,
                         ta_alias=u"உயிர்எழுத்து")
        self.add_builtin("tamil_mei",
                         lambda: tamil.utf8.mei_letters,
                         nargin=0,
                         ta_alias=u"மெய்எழுத்து")
        self.add_builtin("tamil_kuril",
                         lambda: tamil.utf8.kuril_letters,
                         nargin=0,
                         ta_alias=u"குரில்எழுத்து")
        self.add_builtin("tamil_nedil",
                         lambda: tamil.utf8.nedil_letters,
                         nargin=0,
                         ta_alias=u"நேடில்எழுத்து")
        self.add_builtin("tamil_vallinam",
                         lambda: tamil.utf8.vallinam_letters,
                         nargin=0,
                         ta_alias=u"வல்லினம்எழுத்து")
        self.add_builtin("tamil_mellinam",
                         lambda: tamil.utf8.mellinam_letters,
                         nargin=0,
                         ta_alias=u"மெல்லினம்")
        self.add_builtin("tamil_idayinam",
                         lambda: tamil.utf8.idayinam_letters,
                         nargin=0,
                         ta_alias=u"இடைனம்எழுத்து")
        self.add_builtin("tamil_agaram",
                         lambda: tamil.utf8.agaram_letters,
                         nargin=0,
                         ta_alias=u"அகரம்எழுத்து")
        self.add_builtin("tamil_uyirmei",
                         lambda: tamil.utf8.uyirmei_letters,
                         nargin=0,
                         ta_alias=u"உயிர்மெய்எழுத்து")

        self.add_builtin("tamil_istamil_prefix",
                         tamil.utf8.istamil_prefix,
                         nargin=1)
        self.add_builtin("tamil_all_tamil",
                         tamil.utf8.all_tamil,
                         nargin=1,
                         ta_alias=u"தனித்தமிழா")
        self.add_builtin("tamil_hastamil",
                         tamil.utf8.has_tamil,
                         nargin=1,
                         ta_alias=u"தமிழ்கொண்டதா")
        self.add_builtin("tamil_reverse_word",
                         tamil.utf8.reverse_word,
                         nargin=1,
                         ta_alias=u"அந்தாதிமாற்று")
        return True
    ...
    def evaluate(self):
        try:
            if self.SAFE_MODE:
                TransformSafeModeFunctionCheck(interpreter=self,
                                               debug=self.debug)
...
Type "help", "copyright", "credits" or "license" for more information.""" % ezhil_version(
    ...
    def parseline(self, line):
        ...
        line = line.strip()
        if line in [
                u"exit", u"help", u"EOF", u"copyright", u"credits", u"license"
        ]:
            cmd = line
            line = line + u"()"
        return [cmd, arg, line]
    ...
    def default(self, line):
        ...
        try:
            ...
            if (self.debug):
                print(u"*" * 60)
            TransformSemanticAnalyzer(interpreter=self.parse_eval,
                                      debug=self.debug)
            ...
            if hasattr(rval, 'evaluate'):
                pprint(rval.__str__())
                         ...
                         '__str__'):  #print everything except a None object
                pprint(rval)
        except Exception as excep:
            print(u"Exception in code, at line %d,  \"%s\" \n >>>>>>> %s " %
                  (self.line_no - 1, line, unicode(excep)))
            self.lexer.reset()
            if (self.debug):
                raise excep
...
def get_prog_name(lang):
    ...
    parser.add_argument(
        ...
        default=u"UTF-8",
        help=
        u"option to specify other file encodings; supported  encodings are TSCII, and UTF-8"
    )
    parser.add_argument(u"-profile",
                        action=u"store_true",
                        default=None,
                        help=u"profile the input file(s)")
    parser.add_argument(u"-stdin",
                        action=u"store_true",
                        default=None,
    ...
    if not (args.tamilencoding in ["utf-8", "tscii", "TSCII", "UTF-8"]):
        print(
            u"ERROR: Unsupported encoding %s; use values TSCII or UTF-8, or see Help"
            % args.tamilencoding)
        error_fcn()
