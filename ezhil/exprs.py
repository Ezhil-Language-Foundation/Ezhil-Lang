...
class Interpreter(DebugUtils):
    ...
    def __init__(self,lexer, dbg = False):
        ...
        lang_parser = Parser(lexer,self.function_map, \
    ...
    def change_parser(self,parser_ctor):
        ...
        lang_parser = parser_ctor(lexer,self.function_map, \
                             self.builtin_map, self.debug )
        self.parser = lang_parser
        return True
...
if __name__ == "__main__":  
    ## usage: ./exprs.py {-stdin|file-name} {debug-on}
    ...
    if ( fname == "-ezhil" ): 
        ...
        fname = sys.argv[2]
        debug = False
        if ( len(sys.argv) > 2 ): debug = ( sys.argv[2] == "1" )
        ...
        ## rest of it drives the Exprs language
    elif ( fname == "-stdin" or fname == None):
