...
try:
    ...
    print(
        u"Get latest version of open-tamil library using Python Package index as"
    )
...
class EzhilInterpreter(Interpreter):
    ...
    def install_builtins(self):
        ...
        tamil_equiv = {
            u"சரம்_இடமாற்று": u"replace",
            u"சரம்_கண்டுபிடி": u"find",
            u"நீளம்": u"len",
            u"சரம்_உள்ளீடு": u"raw_input",
            u"உள்ளீடு": u"input"
        }
        ...
        tamil_equiv.update({
            ...
            u"பின்இணை": u"append",
            u"தலைகீழ்": u"reverse",
            u"வரிசைப்படுத்து": u"sort",
            u"நீட்டிக்க": u"extend",
            u"நுழைக்க": u"insert",
            u"குறியீட்டெண்": u"index",
            u"வெளியேஎடு": u"pop_list",
            u"பொருந்தியஎண்": u"count"
        })
        ...
        #file operators
        tamil_equiv.update({
            u"கோப்பை_திற": u"file_open",
            u"கோப்பை_மூடு": u"file_close",
            u"கோப்பை_படி": u"file_read",
            u"கோப்பை_எழுது": u"file_write",
            u"கோப்பை_எழுது_வரிகள்": u"file_writelines",
        ...
        turtle_map = {
            u"முன்னாடி": u"forward",
            u"பின்னாடி": u"backward",
            u"வலது": u"lt",
            u"இடது": u"rt",
            u"எழுதுகோல்மேலே": u"penup",
            u"எழுதுகோல்கிழே": u"pendown"
        }
...
class EzhilRedirectOutput:
    ...
    def __init__(self, redirectop, debug=False):
        ...
        self.tmpf = None
        if (self.redirectop):
            self.tmpf = tempfile.NamedTemporaryFile(suffix='.output',
                                                    delete=False)
    ...
    def __init__(self,
        ...
        if (not redirectop):  #run serially and exit.
            try:
                ...
                ezhil_file_parse_eval(file_input,
                                      self.redirectop,
                                      self.debug,
                                      encoding,
                                      doprofile=doprofile,
...
def ezhil_eval(program_text, debug=False, encoding="utf-8", safe_mode=False):
    ...
    if (debug):
        ...
        lexer.dump_tokens()
        print(u"##########################")
    parse_eval = EzhilInterpreter(lexer=lexer,
                                  debug=debug,
                                  safe_mode=safe_mode)
    parse_eval.parse()
...
def ezhil_file_parse_eval(file_input,
                          redirectop,
                          debug,
                          encoding="utf-8",
                          doprofile=False,
...
def execute_file(file_name, debug, encoding, doprofile):
    ...
    """
    try:
        executer = EzhilFileExecuter(file_name,
                                     debug,
                                     encoding=encoding,
                                     doprofile=doprofile)
        executer.run()
...
def main():
    ...
    [fnames, debug, dostdin, encoding, stacksize,
     doprofile] = get_prog_name(lang)
    if dostdin:
        ...
        for idx, file_name in enumerate(fnames):
            if debug:
                print(u" **** Executing file # {} named {} ".format(
                    idx + 1, file_name))
            exit_code = execute_file(file_name=file_name,
                                     debug=debug,
                                     encoding=encoding,
                                     doprofile=doprofile)
        sys.exit(exit_code)
...
def ezhil_timeout_exec(filename):
    return ezhil_file_parse_eval(file_input=filename,
                                 redirectop=False,
                                 debug=False,
                                 encoding="utf-8",
                                 doprofile=False,
